...
class CyphersolLogger {
  ...
  setupLoggers() {
    try {
      ...
      log.transports.file.resolvePathFn = () =>
        path.join(logsDir, "cyphersol-main.log");
  ...
  createSpecializedLoggers(logsDir) {
    try {
      ...
      this.buildLogger.transports.file.resolvePathFn = () =>
      ...
      this.updateLogger.transports.file.resolvePathFn = () =>
        path.join(logsDir, "update.log");
      ...
      this.dbLogger.transports.file.resolvePathFn = () =>
      ...
      this.networkLogger.transports.file.resolvePathFn = () =>
        path.join(logsDir, "network.log");
      ...
      this.perfLogger.transports.file.resolvePathFn = () =>
      ...
      this.errorLogger.transports.file.resolvePathFn = () =>
      ...
      this.userLogger.transports.file.resolvePathFn = () =>
        path.join(logsDir, "user-actions.log");
  ...
  setupLogRotation(logsDir) {
    ...
    try {
      ...
      this.error("SYSTEM", "Failed to clean up old logs", {
        error: error.message,
      });
    }
  }
  ...
  async initializeSystemInfo() {
    try {
      const os = require("os");
      // Use safe Node.js built-in methods instead of systeminformation
      this.systemInfo = {
        os: {
        ...
        network: Object.keys(os.networkInterfaces()).map((name) => ({
          iface: name,
          ip4:
            os.networkInterfaces()[name]?.find((i) => i.family === "IPv4")
              ?.address || "Unknown",
        })),
  ...
  updateCleanup(action, success, details = {}) {
    ...
    this.info(
      "UPDATE",
      `Cleanup ${action}: ${success ? "success" : "failed"}`,
      context
    );
  }
  ...
  dbConnection(action, success, details = {}) {
    ...
    this.info(
      "DATABASE",
      `Database ${action}: ${success ? "success" : "failed"}`,
      context
    );
  }
