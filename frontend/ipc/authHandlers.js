  ...
  ipcMain.handle("license:activate", async (event, args) => {
    ...
    try {
      ...
      if (!alreadyRunning) {
        ...
        // Wait until the server is responsive with extended timeout
        await waitUntilServerIsReady(serverUrl, 30000);
      } else {
...
/**
 * Helper function to get stored user email from global compatibility checker
...
async function getStoredUserEmail() {
  try {
    ...
    log.error("🔌 Error retrieving stored user email:", error.message);
    return null;
  }
}
